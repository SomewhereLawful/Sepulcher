...
/mob/living/proc/apply_damage(damage = 0,damagetype = BRUTE, def_zone = null, blocked = FALSE)
	...
	switch(damagetype)
		...
		if(WILL)
			adjustWill(damage * hit_percent)
		if(HUNGER)
			adjustHunger(damage * hit_percent)
		if(TOXICITY)
			adjustToxicity(damage * hit_percent)
		if(STAMINA)
...
/mob/living/proc/apply_damage_type(damage = 0, damagetype = BRUTE) //like apply damage except it always uses the damage procs
	switch(damagetype)
		...
		if(WILL)
			return adjustWill(damage)
		if(HUNGER)
			return adjustHunger(damage)
		if(TOXICITY)
			return adjustToxicity(damage)
		if(STAMINA)
...
/mob/living/proc/apply_damages(brute = 0, will = 0, hunger = 0, toxicity = 0, def_zone = null, blocked = FALSE, stamina = 0, brain = 0)
	if(blocked >= 100)
		return 0
	if(brute)
